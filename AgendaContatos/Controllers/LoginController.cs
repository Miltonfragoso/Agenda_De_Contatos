    ...
    public IActionResult Index()
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (usuario != null)
                    {
                        ...
                        {
                            _sessao.CriarSessaoSoUsuario(usuario);
        ...
        public IActionResult EnviarLinkParaRedefinirSenha(RedefinirSenhaModel redefinirSenhaModel)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    UsuarioModel usuario = usuarioRepositorio.BuscarPorEmailLogin(redefinirSenhaModel.Email, redefinirSenhaModel.Login);

                    if (usuario != null)
                    {
                        string novaSenha = usuario.GerarNovaSenha();

                        string mensagem = $"Sua senha é: {novaSenha}";
